...
Setup and query config items in environment:
    ...
    >>> # set some env variable with VISION namespace
    >>> os.environ["VISION_PSQL_SERVER"] = "localhost:1234"

    >>> import vision.config
    >>> config.env.getstr("psql_server")
    'localhost:1234'
    >>> config.env.getstr("no_exist")
    vision.config.ConfigValueNotFound: 'no_exist' not found

Setup and query secrets in environment and/or secrets manager:

    >>> # set some env variable with VISION_SECRET namespace
    >>> # OR some code to store "secret" in secretsmanager,
    >>> # with key "<NAMESPACE>-<KEY>"
    >>> os.environ["VISION_SECRET_PASSWORD"] = "secret"
    ...
    def _no_default(self, method, key):
        ...
        if value is None:
            if isinstance(self, _EnvGetter):
                msg = "Please declare VISION_{key_upper} " "in environment.".format(
                    key_upper=key.upper().replace(".", "_"),
                )
            else:
                msg = (
                    "Please declare VISION_SECRET_{key_upper} "
...
class _EnvGetter(_ConfigGetterNoDefaults):
    ...
    def __init__(self):
        super().__init__(
            getconf.finders.NamespacedEnvFinder("vision"),
        )
...
class _SecretGetter(_ConfigGetterNoDefaults):
    ...
    def __init__(self):
        super().__init__(
            getconf.finders.NamespacedEnvFinder("vision_secret"),
